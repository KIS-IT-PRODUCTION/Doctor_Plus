...
export default function PatientMessages() {
  ...
  const fetchMessagesFromSupabase = useCallback(async () => {
    ...
    try {
      ...
      for (const msg of notificationData) {
        ...
        if (bId) {
          const isConsultationCompleted = 
            bookingInfo?.status_meet === true || 
            bookingInfo?.consultation_occurred_patient === true || 
            msg.data?.status_meet === true || 
            msg.notification_type === 'consultation_completed';

          if (!finalProcessedMessages.has(bId) || new Date(msg.created_at) > new Date(finalProcessedMessages.get(bId).created_at)) {
            finalProcessedMessages.set(bId, {
              id: msg.id,
              title: msg.title,
              body: msg.body,
              ...
              consultation_occurred_patient: isConsultationCompleted,
            });
          }
        } else {
          finalProcessedMessages.set(msg.id, {
            id: msg.id,
            title: msg.title,
            body: msg.body,
            created_at: msg.created_at,
      ...
      const finalMessages = Array.from(finalProcessedMessages.values())
  ...
  const handleHoldPayment = useCallback(async (bookingId) => {
    ...
    try {
      ...
      if (!session?.access_token) throw new Error(t('auth_error'));
      ...
      if (!response.ok || !responseData.success) throw new Error(responseData.error || 'Failed to get payment data.');
      
      const { data, signature } = responseData;
      const liqPayUrl = 'https://www.liqpay.ua/api/3/checkout';
      const fullUrl = `${liqPayUrl}?data=${encodeURIComponent(data)}&signature=${encodeURIComponent(signature)}`;
  ...
  const handleConfirmConsultation = useCallback(async (bookingId) => {
    if (!bookingId) return;
    setMessages(prev => prev.map(msg => msg.booking_id === bookingId ? { ...msg, payment_status: 'processing' } : msg));
    ...
    try {
      ...
      const responseData = await response.json();
      if (!response.ok) throw new Error(responseData.error || t('unknown_error'));
  ...
  const handleSubmitFeedback = useCallback(async (bookingId, consultationOccurred, consultationOnTime, starRating, feedbackText) => {
    try {
      ...
      const { error } = await supabase.from('patient_bookings').update({
        consultation_occurred_patient: true,
        ...
        >
          {messages.length === 0 ? (
            ...
            messages.map((message) => {
              ...
              const isCompleted = message.consultation_occurred_patient; 

              const showPayButton = !isAdminAnnouncement && message.booking_status === 'confirmed' && !isPaid && !isHeld && !isProcessing;
              const showJoinMeetButton = !isAdminAnnouncement && (isPaid || isHeld) && message.meet_link && !isCompleted;
              const showConfirmButton = isHeld && !isPaid && !isProcessing;
              const showFeedbackButton = isPaid && isCompleted && !message.has_feedback_patient;
                      ...
                      <>
                        ...
                        {showFeedbackButton && 
                          ...
                          >
                            ...
                            <Text style={styles.feedbackButtonText}>{t('leave_feedback_button') || "Залишити відгук"}</Text>
                          </TouchableOpacity>
                        }

                        {message.has_feedback_patient && 
                          <View style={styles.feedbackLeftButton}>
                             <Ionicons name="checkmark-circle" size={18} color="#757575" style={{marginRight: 6}} />
                             <Text style={styles.feedbackLeftButtonText}>{t('feedback_left') || "Відгук залишено"}</Text>
                          </View>
                        }

                        {isPaid && !showFeedbackButton && !message.has_feedback_patient && !isCompleted &&
...
const styles = StyleSheet.create({
  ...
  feedbackButton: { marginTop: verticalScale(15), backgroundColor: '#FFB300', paddingVertical: verticalScale(12), borderRadius: moderateScale(10), alignItems: 'center', justifyContent: 'center', shadowColor: '#FFB300', shadowOffset: { width: 0, height: verticalScale(4) }, shadowOpacity: 0.3, shadowRadius: moderateScale(5), elevation: 8, flexDirection: 'row' },
  feedbackButtonText: { color: '#FFFFFF', fontSize: moderateScale(16), fontFamily: 'Mont-SemiBold' },
  
  feedbackLeftButton: { marginTop: verticalScale(15), backgroundColor: '#EEEEEE', paddingVertical: verticalScale(12), borderRadius: moderateScale(10), alignItems: 'center', justifyContent: 'center', flexDirection: 'row', borderWidth: 1, borderColor: '#E0E0E0' },
  feedbackLeftButtonText: { color: '#757575', fontSize: moderateScale(15), fontFamily: 'Mont-SemiBold' },
