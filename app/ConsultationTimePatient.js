// ConsultationTimePatient.js - FINAL VERSION with 45-min slots and centered title
...
const ConsultationTimePatient = ({ route }) => {
  ...
  const processAndDisplaySchedule = React.useCallback((availableSlots, bookedSlots, myBookings, ptZone) => {
    ...
    availableSlots.forEach(slot => {
      ...
      const displayDayHeader = localDateTime.toLocaleString(DateTime.DATE_FULL);
      
      if (!scheduleMap.has(displayDateKey)) {
        scheduleMap.set(displayDateKey, { displayDate: displayDayHeader, slots: [] });
      }
      
      const isBooked = bookedSlots.has(slotId);
      const isMyBooking = myBookings.has(slotId);

      // FIX: Calculate end time and format display string
      const endTimeLocalDateTime = localDateTime.plus({ minutes: CONSULTATION_DURATION_MINUTES });
      const displayTime = `${localDateTime.toFormat('HH:mm')} - ${endTimeLocalDateTime.toFormat('HH:mm')}`;
  ...
  const fetchScheduleAndProfile = React.useCallback(async () => {
    ...
    try {
        ...
        const [patientProfileRes, doctorAvailRes, bookingsRes, doctorCostRes] = await Promise.all([
            supabase.from('profiles').select('country_timezone').eq('user_id', currentPatientId).single(),
        ...
        if (!ptZone || !DateTime.local().setZone(ptZone).isValid) {
            ptZone = DateTime.local().zoneName;
        }
        setPatientTimezone(ptZone);

        if (doctorCostRes.error) throw doctorCostRes.error;
        setDoctorConsultationCost(doctorCostRes.data?.consultation_cost ?? 0);

        if (doctorAvailRes.error) throw doctorAvailRes.error;
  ...
  const processBooking = async () => {
    ...
    try {
        ...
        const { error } = await supabase.from('patient_bookings').insert({
            patient_id: patientId,
            doctor_id: doctorId,
            booking_date: dbDate,
            booking_time_slot: dbTime,
            status: 'confirmed',
  ...
  const bookSelectedSlots = () => {
    ...
    if (doctorConsultationCost > 0) {
        Alert.alert(
            t('confirm_booking'),
            `${t('are_you_sure_you_want_to_book_for')} $${doctorConsultationCost.toFixed(2)}?`,
